...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['title']) && isset($_POST['content'])) {
        ...
        if ($title && $content) {
            $stmt = $pdo->prepare("SELECT id FROM about LIMIT 1");
            $stmt->execute();
            $existingRecord = $stmt->fetch(PDO::FETCH_ASSOC);

            if ($existingRecord) {
                $stmt = $pdo->prepare("UPDATE about SET title = :title, content = :content, last_updated = NOW() WHERE id = :id");
                $stmt->execute([':title' => $title, ':content' => $content, ':id' => $existingRecord['id']]);
                echo json_encode(['success' => true, 'message' => 'Record updated successfully.']);
            } else {
                $stmt = $pdo->prepare("INSERT INTO about (title, content, last_updated) VALUES (:title, :content, NOW())");
    ...
    $stmt = $pdo->prepare("SELECT title, content, DATE_FORMAT(last_updated, '%W, %e %M %Y') as last_updated FROM about ORDER BY last_updated DESC LIMIT 1");
    $stmt->execute();
